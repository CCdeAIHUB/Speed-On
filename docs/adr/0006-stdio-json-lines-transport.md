...
5. transport 使用默认、opener、scanner、scanner+opener 四种 dispatcher 组合，不复制 Core 业务逻辑。
...
9. stdio 默认不启用真实应用扫描；只有显式传入 `--enable-application-scan` 时才接入 `speed_on_platform::PlatformApplicationScanner`。
10. 未知启动参数会返回结构化错误，避免前端误以为某个安全开关已生效。
...
- 最小可运行 transport 能让后续前端更早验证 search/recommend/record_selection/open_resource/refresh_applications 流程。
- command opener 是真实本机动作，必须显式启用，不能默认开启。
- 应用扫描会读取用户系统应用目录，必须显式启用，不能默认开启。
...
### 方案 C：先做 stdio JSON Lines + 平台能力 opt-in

这是当前选择。它最小、跨平台、易测试，默认安全关闭真实打开和应用扫描动作，并且不会阻止后续替换为更原生的 IPC transport。
...
- 新增 `--enable-application-scan`。
- 新增/更新 stdio transport tests。
...
- JSON Lines 不适合高频大流量数据流，但当前 search/recommend/record_selection/open_resource/refresh_applications 足够。
...
- 启用 application scan 后会读取系统应用目录，后续需要用户设置和首次运行策略。
