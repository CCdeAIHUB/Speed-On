...
5. transport 使用 `JsonIpcDispatcher`，不复制 search/recommend/record_selection 业务逻辑。
...
- 最小可运行 transport 能让后续前端更早验证 search/recommend/record_selection 流程。
...
### 方案 C：先做 stdio JSON Lines

这是当前选择。它最小、跨平台、易测试，并且不会阻止后续替换为更原生的 IPC transport。
...
- JSON Lines 不适合高频大流量数据流，但当前 search/recommend/record_selection 足够。
