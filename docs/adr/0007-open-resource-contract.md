...
8. 默认 `speed-on-ipc-stdio` binary 仍不接入真实平台 opener，因此 open_resource 返回 unsupported。
9. 真实平台 opener 后续必须作为 Windows/macOS/Linux adapter 单独实现和测试。
...
### 方案 C：Core 定义契约，平台 adapter 实现 opener

这是当前选择。Core 统一 API 和错误结构，真实平台 opener 后续独立实现。
...
- 当前还没有真实平台 opener，因此 stdio binary 的 open_resource 会返回 unsupported。
...
5. 增加目标存在性校验和 URL scheme 白名单。
6. 将成功打开动作写入 activity_records / resource_usage_stats。
7. 将 opener 错误写入脱敏 system_logs。
