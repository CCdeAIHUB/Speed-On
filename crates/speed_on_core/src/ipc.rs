        ...
        + UserOperationLogRepository,
{
    ...
    fn dispatch_inner(&mut self, request: &IpcRequest) -> ApiResponse<Value> {
        ...
        match request.command {
            IpcCommand::Search => {
                dispatch_search(&mut self.api, request, "ipc::JsonIpcDispatcher::search")
            }
            IpcCommand::Recommend => {
                dispatch_recommend(&self.api, request, "ipc::JsonIpcDispatcher::recommend")
            }
            ...
            IpcCommand::RefreshApplications => {
                ApiResponse::failure(AppError::platform_unsupported(
    ...
    O: ResourceOpener,
{
    ...
    fn dispatch_inner(&mut self, request: &IpcRequest) -> ApiResponse<Value> {
        ...
        match request.command {
            IpcCommand::Search => dispatch_search(
                &mut self.api,
                request,
                "ipc::JsonIpcDispatcherWithOpener::search",
            ),
            IpcCommand::Recommend => dispatch_recommend(
                &self.api,
                request,
            ...
            IpcCommand::OpenResource => match decode_payload::<ApiOpenResourceRequest>(
                ...
                Ok(payload) => {
                    api_response_to_json(self.api.open_resource_with(&mut self.opener, payload))
                }
    ...
    fn dispatch_inner(&mut self, request: &IpcRequest) -> ApiResponse<Value> {
        ...
        match request.command {
            IpcCommand::Search => dispatch_search(
                &mut self.api,
                request,
                "ipc::JsonIpcDispatcherWithScanner::search",
            ),
            IpcCommand::Recommend => dispatch_recommend(
                &self.api,
                request,
                "ipc::JsonIpcDispatcherWithScanner::recommend",
            ),
            IpcCommand::RecordSelection => dispatch_record_selection(
                &mut self.api,
                request,
            ...
            IpcCommand::RefreshApplications => {
                match decode_payload::<ApiRefreshApplicationsRequest>(
                    request.payload.clone(),
                    "ipc::JsonIpcDispatcherWithScanner::refresh_applications",
                ) {
                    Ok(payload) => api_response_to_json(
                        self.api.refresh_applications_with(&self.scanner, payload),
                    ),
                    Err(error) => ApiResponse::failure(error),
                }
            }
    ...
    fn dispatch_inner(&mut self, request: &IpcRequest) -> ApiResponse<Value> {
        ...
        match request.command {
            IpcCommand::Search => dispatch_search(
                &mut self.api,
                request,
                "ipc::JsonIpcDispatcherWithScannerAndOpener::search",
            ),
            IpcCommand::Recommend => dispatch_recommend(
                &self.api,
                request,
                "ipc::JsonIpcDispatcherWithScannerAndOpener::recommend",
            ),
            IpcCommand::RecordSelection => dispatch_record_selection(
                &mut self.api,
                request,
...
fn dispatch_record_selection<R>(
    api: &mut CoreApi<R>,
    request: &IpcRequest,
    module: &'static str,
) -> ApiResponse<Value>
